...
the initial unused-bits byte. If we then go further down, the new start is the current start + it's header length + 1.
...
hl              : header length. 0 for offset, hl at start for strparse.
